...
class Calendar:
    ...
    async def _build_calendar_body(
        ...
        """
        calendarBody: BaseCalendarModel = {
            "summary": title,
            "location": location,
            "description": description,
    ...
    async def get_events(
        ...
        response: EventsModel = await execute_aiogoogle(
            method_callable=method_callable,
            service_account_credentials=self.service_account_credentials,
            api_name=self.api_name,
            api_version=self.api_version,
    ...
    async def add_share_rule(
        ...
        rule = await self.build_scope_body(scope_type, user, role)
    ...
    async def update_share_rule(
        ...
        newRule = await self.build_scope_body(scope_type, user, role)
